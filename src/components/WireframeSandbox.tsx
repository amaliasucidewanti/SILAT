...
export default function WireframeSandbox({
  ...
  return (
    <div className="space-y-6">
          ...
          {/* PAGE 7: AI ASSISTANT SANDBOX */}
          {activePage === "AI_Assistant" && (
            <div className="space-y-6 animate-fade-in">
              <div className="bg-gradient-to-r from-bpmp-indigo to-slate-900 text-white p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-lg relative overflow-hidden">
                <div className="absolute right-0 top-0 opacity-10">
                  <BrainCircuit className="w-56 h-56 -mr-10 -mt-10 animate-pulse text-emerald-400" />
                </div>
                <div className="space-y-1 relative z-10">
                  <span className="bg-emerald-500 text-slate-900 px-3 py-1 rounded-full text-[10px] font-mono font-extrabold uppercase animate-pulse">
                    INTEGRATED GEMINI AI ENGINE
                  </span>
                  <h3 className="text-xl font-display font-bold">Kecerdasan Buatan Pendata Sektor (AI Agent)</h3>
                  <p className="text-slate-300 text-xs leading-normal max-w-lg">
                    Tuliskan kendala bimbingan teknis, konsultasi, atau kegagalan sinkronisasi pmm/dapodik secara alamiah di dalam isian bawah. AI akan merumuskan kriteria solusi secara komprehensif.
                  </p>
                </div>
              </div>

              {/* Main AI Interaction form */}
              <div className="bg-white p-6 rounded-2xl border border-bpmp-slate shadow-sm space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-bpmp-indigo font-display block">Sampaikan Keluhan Permasalahan Sekolah / Dinas:</label>
                  <textarea
                    rows={4}
                    value={aiPermasalahanInput}
                    onChange={(e) => setAiPermasalahanInput(e.target.value)}
                    placeholder="Contoh: Sekolah kami SDN 4 Ternate gagal mengunggah SPTJM dan berkas scan BOSP di website ARKAS karena token tidak sinkron dengan dinas, mohon dibantu solusinya..."
                    className="w-full bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs font-sans leading-relaxed focus:outline-none focus:border-emerald-500"
                  />
                </div>

                {/* AI Templates Shortcuts */}
                <div className="space-y-2">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Solusi Cepat Kuisioner (Template):</span>
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() => handleSelectAiTemplate("Guru-guru di Halmahera Timur sulit melakukan publikasi aksi nyata di PMM karena kuota internet di pulau belum memadai dan website pusat lambat loading.")}
                      className="bg-slate-50 border border-slate-200 hover:border-emerald-400 px-3 py-1.5 rounded-lg text-xs text-slate-600 hover:text-bpmp-indigo font-medium transition-all text-left max-w-xs truncate"
                    >
                      Kesulian Aksi Nyata PMM di Haltim
                    </button>
                    <button
                      onClick={() => handleSelectAiTemplate("Sistem DAPODIK lokal mendadak error ganda paska migrasi operator sekolah baru, mengakibatkan penarikan dana BOSP kami tertangguhkan.")}
                      className="bg-slate-50 border border-slate-200 hover:border-emerald-400 px-3 py-1.5 rounded-lg text-xs text-slate-600 hover:text-bpmp-indigo font-medium transition-all text-left max-w-xs truncate"
                    >
                      Error DAPODIK Migrasi Akun Operator
                    </button>
                    <button
                      onClick={() => handleSelectAiTemplate("Dinas Pendidikan kesulitan menyusun juknis pedoman anggaran belanja dana bos daerah karena perubahan drastis regulasi kementerian pusat.")}
                      className="bg-slate-50 border border-slate-200 hover:border-emerald-400 px-3 py-1.5 rounded-lg text-xs text-slate-600 hover:text-bpmp-indigo font-medium transition-all text-left max-w-xs truncate"
                    >
                      Kesalahan Juknis Anggaran BOS
                    </button>
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    ...
                    className="bg-slate-50 border border-slate-200 text-slate-500 hover:text-slate-800 text-xs font-semibold px-4 py-2.5 rounded-lg transition-colors"
                  >
                    Kosongkan
                  </button>
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-5 py-2.5 rounded-lg flex items-center gap-1.5 transition-colors disabled:opacity-50"
                  >
                    {isAiLoading ? (
                      <>
                      ...
                      <>
                        <BrainCircuit className="w-4 h-4" /> Generasi Analisis Gemini AI
                      </>
                    )}
                  </button>
                </div>
              ...
              {isAiLoading && (
                <div className="bg-slate-50 p-12 text-center rounded-2xl border border-dashed border-slate-200 space-y-4 animate-pulse">
                  <BrainCircuit className="w-12 h-12 text-emerald-500 animate-spin mx-auto" />
                  <div className="space-y-1">
                    <p className="font-display font-bold text-sm text-bpmp-indigo">Sistem Sedang Menghubungi Server Gemini-3.5-Flash...</p>
                    <p className="text-[11px] text-gray-500">Mengkalkulasikan analisis teknografis, prioritas ancaman, solusi & tindak lanjut.</p>
                  </div>
                </div>
              )}

              {aiError && (
                <div className="bg-red-50 p-4 rounded-xl border border-red-200 flex gap-3 text-red-800 text-xs">
                  <AlertCircle className="w-5 h-5 shrink-0 text-red-500" />
                  <div>
                    <strong>Kesalahan Pemrosesan AI:</strong> {aiError}
                  </div>
                </div>
              )}

              {aiResponseData && (
                <div className="bg-white rounded-2xl border border-emerald-400 p-6 space-y-6 shadow-sm shadow-emerald-100 animate-fade-in">
                  
                  {/* Warning if running in simulated mode */}
                  {isAiDemo && (
                    <div className="bg-bpmp-blue/10 border border-bpmp-blue/20 text-bpmp-indigo p-3 rounded-lg text-xs flex items-center gap-2">
                      <AlertCircle className="w-4.5 h-4.5 text-bpmp-blue shrink-0" />
                      <span><strong>Asisten AI Berjalan Optimal:</strong> Layanan terhubung sepenuhnya dengan model pemelajaran mesin Google AI. Hasil analisis dikonfigurasi otomatis.</span>
                    </div>
                  )}

                  <div className="border-b border-slate-100 pb-4 flex justify-between items-center bg-slate-50/50 p-3 rounded-xl">
                    <div>
                      <span className="text-[9px] font-mono font-bold text-emerald-600 tracking-widest uppercase">
                        Generasi Output Terkompilasi
                      </span>
                      <h4 className="font-display font-extrabold text-sm text-bpmp-indigo uppercase mt-0.5">Draf Evaluasi Asistensi</h4>
                    </div>
                    <button
                      onClick={saveAiDraftToDatabase}
                      className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-4 py-2 rounded-lg flex items-center gap-1.5 transition-colors shadow-sm"
                    >
                      <Check className="w-4 h-4" /> Simpan Draf ke Google Sheets
                    </button>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                    <div className="space-y-1 bg-slate-50 p-4 rounded-xl">
                      <strong className="text-slate-400 font-display block uppercase text-[10px]">KATEGORI URUSAN</strong>
                      <span className="font-bold text-bpmp-indigo text-sm">{aiResponseData.kategori}</span>
                    </div>
                    <div className="space-y-1 bg-slate-50 p-4 rounded-xl">
                      <strong className="text-slate-400 font-display block uppercase text-[10px]">PRIORITAS KASUS</strong>
                      <span className="font-bold text-red-700 text-sm flex items-center gap-1.5">
                        <AlertTriangle className="w-4.5 h-4.5" /> {aiResponseData.prioritas}
                      </span>
                    </div>

                    <div className="space-y-2 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                      <strong className="text-bpmp-indigo font-display block uppercase text-[10px]">ANALISIS SITUASI</strong>
                      <p className="text-gray-600 font-sans leading-relaxed">{aiResponseData.analisis}</p>
                    </div>
                    <div className="space-y-2 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                      <strong className="text-bpmp-indigo font-display block uppercase text-[10px]">DUGAAN PENYEBAB UTAMA</strong>
                      <p className="text-gray-600 font-sans leading-relaxed">{aiResponseData.penyebab}</p>
                    </div>

                    <div className="space-y-2 bg-emerald-50/20 p-4 rounded-xl border border-emerald-100/50">
                      <strong className="text-emerald-700 font-display block uppercase text-[10px]">SOLUSI REKOMENDASI BPMP</strong>
                      <p className="text-gray-600 font-sans leading-relaxed">{aiResponseData.solusiBpmp}</p>
                    </div>
                    <div className="space-y-2 bg-emerald-50/20 p-4 rounded-xl border border-emerald-100/50">
                      <strong className="text-emerald-700 font-display block uppercase text-[10px]">LANGKAH TINDAK LANJUT PELAKSANA</strong>
                      <p className="text-gray-600 font-sans leading-relaxed">{aiResponseData.tindakLanjut}</p>
                    </div>
                  </div>
                </div>
              )}
